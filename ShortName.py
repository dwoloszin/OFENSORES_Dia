...
def tratarShortNumber(df,siteNameCollumn):# 'Tecnologia','Station'
    df2 = df.copy()
    df2.loc[np.array(list(map(len,df2[siteNameCollumn].values))) == 11,'SHORT'] = df2[siteNameCollumn].str[4:]
    ...
    #print(df3)
    #df2.loc[(df2[Tecnologia] == '2G') & (np.array(list(map(len,df2[siteNameCollumn].astype(str).values))) == 6),'SHORT'] = df2[siteNameCollumn]
    #todo incluir shot 3G puro
    #df2.loc[(df2[Tecnologia] == '3G') & (np.array(list(map(len,df2[siteNameCollumn].astype(str).values))) == 6),'SHORT'] = df2[siteNameCollumn]   
    #df2 = Short2Gto3G(df2,Tecnologia,Station)
    df2.loc[(df2[siteNameCollumn].str[2:3] == '-') & (np.array(list(map(len,df2[siteNameCollumn].astype(str).values))) == 9),'SHORT'] = df2[siteNameCollumn].str[3:]
    #df2.loc[(df2[siteNameCollumn].str[:3] == '4D-') & (np.array(list(map(len,df2[siteNameCollumn].astype(str).values))) == 12),'SHORT'] = df2[siteNameCollumn].str[2:-2]
    #  3SLBUCP03 
...
def tratarShortNumber2(df,siteNameCollumn,Tecnologia,Station):
    ...
    df2.loc[(df2[siteNameCollumn].str[:2] == 'SL') & (np.array(list(map(len,df2[siteNameCollumn].astype(str).values))) == 11),'Short1'] = df2[siteNameCollumn].str[2:-2]
