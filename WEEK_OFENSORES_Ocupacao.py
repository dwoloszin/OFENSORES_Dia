...
def processArchive(Agregacao):
    fields = ['Semana',Agregacao,'ANF','Station ID','BTS/NodeB/ENodeB','Celula','CRITICO']
    fields2 = ['Semana',Agregacao,'ANF','Station ID','BTS/NodeB/ENodeB','Celula','CRITICO']
    ...
    frameSI = frameSI.loc[(~frameSI[Agregacao].isna()) & (frameSI['CRITICO'].astype(float) > 0)]
    ...
    csv_path = os.path.join(script_dir, 'export/'+Agregacao+'_W_Ofensor/'+ 'Ofensores.csv')
